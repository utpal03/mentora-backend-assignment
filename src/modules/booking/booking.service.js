...
export async function createBooking(parentId, { studentId, lessonId }) {
  ...
  const student = await bookingRepository.findStudentById(sid);
  if (!student) {
  ...
  const existing = await bookingRepository.findExistingBooking(sid, lid);
  if (existing) {
    const error = new Error('Student is already booked for this lesson');
    error.statusCode = 409;
    throw error;
  }
  return bookingRepository.create({ studentId: sid, lessonId: lid });
}
